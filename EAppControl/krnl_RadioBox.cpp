...
QStringList krnl_RadioBox::取横向对齐方式列表()
{
	QStringList Items = {
	QStringLiteral("左边"),
	QStringLiteral("居中"),
...
QStringList krnl_RadioBox::取纵向对齐方式列表()
{
	QStringList Items = {
	QStringLiteral("顶边"),
	QStringLiteral("居中"),
...
void krnl_RadioBox::取控件默认附加属性(QHash<QString, QVariant>& out_data)
{
	out_data[QStringLiteral("按钮形式")] = 0;
	out_data[QStringLiteral("平面")] = 0;
	out_data[QStringLiteral("标题")] = QStringLiteral("单选框");
...
void krnl_RadioBox::反序列化控件附加属性(unsigned char* pUnitDataPtr, QHash<QString, QVariant>& out_data)
{
...
void krnl_RadioBox::显示控件属性信息(QHash<QString, QVariant>& map_ControlData)
{
	...
	EAppControl::添加列表控件(QStringLiteral("横向对齐方式"), krnl_RadioBox::取横向对齐方式列表(), map_ControlData[QStringLiteral("横向对齐方式")].toUInt());
	EAppControl::添加列表控件(QStringLiteral("纵向对齐方式"), krnl_RadioBox::取纵向对齐方式列表(), map_ControlData[QStringLiteral("纵向对齐方式")].toUInt());
...
qstring krnl_RadioBox::取事件名称(int eventIndex)
{
	qstring ret;
	switch (eventIndex)
	{
...
QString krnl_RadioBox::取图标资源路径()
{
	return QStringLiteral(":/ICO/ICO/单选框.ico");
}